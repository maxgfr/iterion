...
The same `.iter` file produces the same graph of nodes and edges every time. The compiler ([pkg/dsl/ir/compile.go](../pkg/dsl/ir/compile.go)) takes the AST to an IR; the validator ([pkg/dsl/ir/validate.go](../pkg/dsl/ir/validate.go)) emits diagnostic codes C001–C072 for structural problems *before* you spend a token.
