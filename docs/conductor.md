# `iterion conduct` — long-running dispatcher

The conductor turns iterion from a one-shot `iterion run` into a
...
need the conductor — see [`docs/native-tracker.md`](native-tracker.md)
...
# 2. Write an `iterion.conductor.yaml` next to your workflow.
cat > iterion.conductor.yaml <<'EOF'
name: dev-loop
...
moved out of the eligible set — fine, the conductor releases without
re-dispatching).
...
These conductor workspaces are **distinct** from the engine's per-run
`worktree: auto` — the latter is the runtime's git-isolation
mechanism and lives **inside** the conductor workspace. Both layers
...
Hooks execute via `sh -lc` with `cwd=<workspace path>`. The conductor
...
| `{{conductor.name}}`            | the `name:` from your config               |
| `{{conductor.run_id}}`          | the dispatch's run ID                      |
| `{{conductor.workspace_path}}`  | absolute workspace path                    |
| `{{conductor.attempt}}`         | 0 on first try, N for the (N+1)th retry    |
...
tracker:
  ...
  github:
    ...
    include_labels: [conductor-eligible]
...
The conductor's `Claim` adds `iterion-claimed`; `Release` removes it.
...
The `server.port` setting starts the conductor's HTTP server (the
same SPA the editor serves, so you get the kanban + dashboard at
`http://localhost:<port>`).

| Endpoint                                            | Method | Description                              |
|-----------------------------------------------------|--------|------------------------------------------|
| `/api/v1/conductor/state`                           | GET    | Live snapshot (running, retries, slots). |
| `/api/v1/conductor/refresh`                         | POST   | Force an immediate tick.                 |
| `/api/v1/conductor/reload`                          | POST   | Re-parse the YAML config.                |
| `/api/v1/conductor/issues/{id}`                     | GET    | Per-issue conductor view.                |
| `/api/v1/conductor/issues/{id}/cancel`              | POST   | Cancel an in-flight run.                 |
| `/api/v1/conductor/ws`                              | WS     | Snapshot stream (push on each tick).     |
| `/api/v1/native/*`                                  | —      | Kanban store CRUD (when native is wired).|
| `/api/server/info`                                  | GET    | SPA bootstrap (flags `conductor_enabled`, `native_tracker_enabled`). |

## Single-instance safety

The conductor refuses to start a second instance against the same
workspace root: it holds an exclusive flock on
`<workspace.root>/.conductor.lock` for its lifetime.

For multiple conductors against the same **tracker** but different
filesystems (e.g. dev laptop + CI), the per-issue claim marker
(`iterion-claimed` label on GH/Forgejo, `claim:` field on native)
prevents simultaneous dispatch — each conductor writes its own marker
and refuses to dispatch issues marked by anyone else.

## Operational tips

- Always pair `iterion conduct` with `iterion editor` (or just visit
...
- For headless / containerized deployments, set `server.port: 0` and
  scrape `/api/v1/conductor/state` via Prometheus' `json_exporter` or
  similar.
...
- The conductor does not auto-transition issues on success. Your
...
- Cross-tracker fan-in (one conductor watching GitHub + Linear at once).
