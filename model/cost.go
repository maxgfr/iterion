...
// EstimateCostUSD returns a rough cost estimate for the given token
// usage on the named model. Returns 0 when the model is not in the
// price table — callers should treat 0 as "unknown" and skip emission.
//
...
// annotateUsage writes the conventional `_tokens` / `_model` /
// `_cost_usd` keys onto a generation output. Cost is omitted when the
// model is unknown to the price table, so observers can distinguish
// "no cost data" from "$0".
func annotateUsage(output map[string]interface{}, model string, inputTokens, outputTokens int) (totalTokens int) {
	totalTokens = inputTokens + outputTokens
	output["_tokens"] = totalTokens
	output["_model"] = model
	if cost := EstimateCostUSD(model, inputTokens, outputTokens); cost > 0 {
