...
// ConfigWatcher tails a conductor config file. When the file changes,
...
func (w *ConfigWatcher) loop(onReload func(*Config)) {
	for {
		...
		case err, ok := <-w.fsWatcher.Errors:
			...
			w.logger.Warn("conductor watcher: %v", err)
		}
	}
}
...
func (w *ConfigWatcher) scheduleReload(onReload func(*Config)) {
	...
	w.debounce = time.AfterFunc(configDebounce, func() {
		cfg, err := Load(w.path)
		if err != nil {
			w.logger.Warn("conductor: config reload failed, keeping previous: %v", err)
			return
		}
		w.logger.Info("conductor: config reloaded (%s)", w.path)
		onReload(cfg)
	})
	w.mu.Unlock()
}
