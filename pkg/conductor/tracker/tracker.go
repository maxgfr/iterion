...
// conductor (`iterion conduct`). The conductor polls a Tracker
...
// conductor boundary. Adapter-specific fields go into Metadata; native
// custom fields go into Fields.
type Issue struct {
	// ID is the stable, globally-unique identifier the conductor uses
...
// Implementations should be safe for concurrent calls — the conductor
...
type Tracker interface {
	...
	// indicative; the conductor re-sorts by Priority + CreatedAt.
	...
	// the conductor to leave a trail of dispatch/finish events. Adapters
