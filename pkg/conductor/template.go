...
// It supports {{issue.path}} and {{conductor.path}} references with the
...
type templateSegment struct {
	...
	namespace string   // "issue" | "conductor" when this is a reference
...
func parseTemplateRef(expr, raw string) (templateSegment, error) {
	...
	case "conductor":
		if !isKnownConductorPath(parts[1:]) {
			return templateSegment{}, fmt.Errorf("invalid reference %s: unknown conductor field %q", raw, strings.Join(parts[1:], "."))
		}
	default:
		return templateSegment{}, fmt.Errorf("invalid reference %s: unknown namespace %q (issue | conductor)", raw, ns)
	}
...
func isKnownConductorPath(p []string) bool {
	if len(p) != 1 {
		return false
	}
	switch p[0] {
...
// ConductorVars exposes per-dispatch context to templates.
type ConductorVars struct {
	Name          string
	RunID         string
...
func resolveRef(ns string, path []string, ctx TemplateContext) (string, error) {
	...
	case "conductor":
		return resolveConductor(path, ctx.Conductor)
	}
...
func resolveConductor(path []string, c ConductorVars) (string, error) {
	if len(path) != 1 {
		return "", fmt.Errorf("invalid conductor path %v", path)
	}
	switch path[0] {
	case "name":
		return c.Name, nil
	...
	return "", fmt.Errorf("unknown conductor field %q", path[0])
}
