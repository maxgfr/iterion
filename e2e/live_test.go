...
// ---------------------------------------------------------------------------
// Live E2E test — MCP mode (goai + MCP tools) carte stellaire
// ---------------------------------------------------------------------------

// TestLive_DualModel_PlanImplementReview_MCP is the MCP variant of the carte
// stellaire test. Instead of delegating to CLI subprocesses, it uses a single
// supervisor model (GPT-5.4 via OPENAI_API_KEY) that orchestrates work through
// MCP tool servers (claude mcp serve + codex mcp-server).
//
// This provides better observability: every tool call is a traced event.
//
// Requires:
//   - OPENAI_API_KEY set (for goai → GPT-5.4)
//   - `claude` CLI in PATH (for `claude mcp serve`)
//   - `codex` CLI in PATH (for `codex mcp-server`)
//
// Automatically skipped when API keys or CLIs are absent or in -short mode.
func TestLive_DualModel_PlanImplementReview_MCP(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping live test in short mode")
	}
	loadDotEnv(t)
	...
	// Default supervisor model if not set via .env or environment.
	if os.Getenv("SUPERVISOR_MODEL") == "" {
		t.Setenv("SUPERVISOR_MODEL", "openai/gpt-5.4")
	}

	// Compile the MCP fixture.
	wf := compileFixture(t, "dual_model_plan_implement_review_mcp.iter")

	// Create a persistent workspace directory.
	workspaceDir, err := os.MkdirTemp("", "iterion-plan-impl-review-mcp-*")
	if err != nil {
		t.Fatalf("Failed to create workspace dir: %v", err)
	}
	t.Logf("Workspace directory (persists after test): %s", workspaceDir)
	...
	runID := "live-plan-impl-review-mcp"
	...
	// Build executor options: MCP manager + tool registry for MCP tools mode.
	// Also keep delegate registry as fallback for any remaining delegate nodes.
	execOpts := []model.GoaiExecutorOption{
		model.WithDelegateRegistry(delegate.DefaultRegistry()),
		model.WithWorkDir(workspaceDir),
		model.WithEventHooks(hooks),
	...
	// 5-hour timeout — planning + validation + implementation + review phases.
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Hour)
	defer cancel()
	inputs := map[string]interface{}{
		"task_description":    taskDescription,
	...
	t.Log("Starting live dual-model MCP plan/implement/review workflow run...")
	start := time.Now()
	runErr := eng.Run(ctx, runID, inputs)
	elapsed := time.Since(start)
	t.Logf("Run completed in %s", elapsed.Round(time.Second))
	...
	// Verify both tool perspectives were invoked (claude_code + codex MCP tools).
	claudeNodeCalled := false
	codexNodeCalled := false
	for _, id := range finishedNodes {
		if strings.Contains(id, "claude") {
	...
	if !claudeNodeCalled {
		t.Error("No claude_* node finished — claude_code MCP tools may have failed")
	}
	if !codexNodeCalled {
		t.Error("No codex_* node finished — codex MCP tools may have failed")
	}
	...
	// Count tool call events for MCP observability.
	toolCallCount := countEventType(events, store.EventToolCalled)
	t.Logf("MCP tool calls: %d", toolCallCount)
	...
	// Collect and log metrics.
	metrics, mErr := benchmark.CollectMetrics(s, runID, "live-plan-impl-review-mcp", "")
	if mErr != nil {
		t.Fatalf("Failed to collect metrics: %v", mErr)
	}
...
func requireEnv(t *testing.T, name string) {
	t.Helper()
	if os.Getenv(name) == "" {
		t.Skipf("%s not set — skipping live MCP test", name)
	}
}
