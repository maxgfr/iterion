// E2E coverage for the conductor layer: native tracker + adapter +
// actor + dispatch end-to-end against a StubRunner. No external CLI,
// no LLM, no network — just the iterion conductor pipeline.
...
// newConductorFixture wires a native tracker + workspaces + StubRunner
// + Conductor on a temporary directory. The returned cleanup function
// stops the actor and removes timers.
func newConductorFixture(t *testing.T, polling time.Duration) (
	*conductor.Conductor,
	*native.Store,
	*conductor.StubRunner,
	func(),
) {
	t.Helper()
	dir := t.TempDir()

	ns, err := native.NewStore(dir + "/conductor")
	if err != nil {
		t.Fatalf("NewStore: %v", err)
	}

	ws, err := conductor.NewWorkspaces(dir + "/conductor/workspaces")
	if err != nil {
	...
	cfg := &conductor.Config{
		...
		Tracker:   conductor.TrackerConfig{Kind: "native"},
		Polling:   conductor.PollingConfig{IntervalMS: int(polling.Milliseconds())},
		Agent:     conductor.AgentConfig{MaxConcurrent: 2, MaxRetryBackoffMS: 500},
		Workspace: conductor.WorkspaceConfig{Root: dir + "/conductor/workspaces"},
		Stall:     conductor.StallConfig{TimeoutMS: 0},
	}
...
func TestConductorE2E_DispatchAndRelease(t *testing.T) {
	dispatched := make(chan conductor.DispatchSpec, 4)
	...
	var got conductor.DispatchSpec
	select {
	case got = <-dispatched:
	case <-time.After(2 * time.Second):
		t.Fatal("dispatch never fired")
	}
	if !strings.HasPrefix(got.RunID, "conductor-") {
	...
	t.Fatalf("conductor did not release claim. snapshot=%+v", c.Snapshot())
}

func TestConductorE2E_RetryAfterFailure(t *testing.T) {
	var calls atomic.Int32
	c, ns, runner, cleanup := newConductorFixture(t, 50*time.Millisecond)
	defer cleanup()

	runner.Handler = func(_ context.Context, _ conductor.DispatchSpec) error {
		calls.Add(1)
...
func TestConductorE2E_CancelInFlight(t *testing.T) {
	c, ns, runner, cleanup := newConductorFixture(t, 50*time.Millisecond)
	defer cleanup()

	started := make(chan struct{}, 1)
	runner.Handler = func(ctx context.Context, _ conductor.DispatchSpec) error {
		started <- struct{}{}
		<-ctx.Done()
		return ctx.Err()
	}
...
func TestConductorE2E_RespectsTerminalStateChange(t *testing.T) {
	c, ns, runner, cleanup := newConductorFixture(t, 50*time.Millisecond)
	defer cleanup()
	...
	// Externally move issue to a terminal state — conductor should cancel.
	...
	t.Fatalf("conductor did not honor external state change")
}

func TestConductorE2E_HTTPSurface(t *testing.T) {
	c, ns, runner, cleanup := newConductorFixture(t, 50*time.Millisecond)
	defer cleanup()

	runner.Handler = func(_ context.Context, _ conductor.DispatchSpec) error { return nil }
	...
	// Conductor: refresh tick + state.
	if r, err := http.Post(srv.URL+"/api/v1/conductor/refresh", "", nil); err != nil || r.StatusCode != http.StatusAccepted {
...
// Sanity check that the conductor's snapshot stays JSON-stable across
// ticks even when nothing is dispatched.
func TestConductorE2E_SnapshotShape(t *testing.T) {
	c, _, _, cleanup := newConductorFixture(t, 50*time.Millisecond)
	defer cleanup()
	snap := c.Snapshot()
