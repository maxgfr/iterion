...
export default function BreadcrumbBar() {
  ...
  return (
    <div className="absolute top-2 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1 bg-gray-900/90 border border-gray-700 rounded-lg px-3 py-1.5 shadow-lg backdrop-blur-sm">
      {/* Back button */}
      <button
        className="text-gray-400 hover:text-white text-sm mr-1 px-1"
      ...
      {subNodeViewStack.map((nodeId, index) => (
        <span key={index} className="flex items-center gap-1">
          ...
          {index < subNodeViewStack.length - 1 ? (
            ...
            <span className="text-xs text-white font-medium">{nodeId}</span>
          )}
        </span>
      ))}
    </div>
