...
// SettingsDrawer is the form-driven editor for conductor.json. It
// loads the current config from /api/v1/conductor/config on open and
// PUTs the result on save. No YAML editing required.
export default function SettingsDrawer({ open, onClose, onSaved }: Props) {
  const [cfg, setCfg] = useState<conductor.ConductorConfig | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const c = await conductor.getConfig();
  ...
  const onSave = async () => {
    ...
    setError(null);
    try {
      await conductor.saveConfig(cfg);
      onSaved();
      onClose();
    } catch (e) {
...
function defaultConfig(): conductor.ConductorConfig {
  return {
    name: "",
    workflow: "",
    tracker: { kind: "native" },
...
interface FormProps {
  ...
  setCfg: (c: conductor.ConductorConfig) => void;
}

function Form({ cfg, setCfg }: FormProps) {
  const update = <K extends keyof conductor.ConductorConfig>(
    k: K,
    v: conductor.ConductorConfig[K],
  ) => setCfg({ ...cfg, [k]: v });
    ...
    <div className="space-y-6">
      <Section title="General">
        ...
        >
          <input
            ...
            placeholder="./examples/bots/vibe_feature_dev.bot"
          />
        </Field>
      </Section>
      ...
      <Section title="Tracker">
        <Field label="Kind">
          <select
            ...
            onChange={(e) => update("tracker", { ...cfg.tracker, kind: e.target.value as conductor.TrackerKind })}
          >
      ...
      <Section title="Dispatch — workflow vars">
        <p className="text-xs text-fg-muted">
          ...
          <code>{"{{conductor.workspace_path}}"}</code>, etc.
        </p>
        <KVTable
      ...
      <Section title="Workspace">
        <Field
          label="Root"
          hint="Per-issue workspaces live here. Empty = <store-dir>/conductor/workspaces."
        >
          <input
            className="input"
            value={cfg.workspace?.root ?? ""}
            onChange={(e) => update("workspace", { ...(cfg.workspace ?? {}), root: e.target.value })}
            placeholder="~/.iterion/conductor-workspaces"
          />
        </Field>
        <Field label="Persistence">
          <select
            ...
            onChange={(e) =>
              update("workspace", {
                ...(cfg.workspace ?? {}),
                persist: e.target.value as conductor.WorkspacePersistPolicy,
              })
            }
          >
...
function GitHubTrackerFields({
  ...
  cfg: conductor.GitHubTrackerConfig;
  set: (c: conductor.GitHubTrackerConfig) => void;
}) {
  return (
    <>
...
function ForgejoTrackerFields({
  ...
  cfg: conductor.ForgejoTrackerConfig;
  set: (c: conductor.ForgejoTrackerConfig) => void;
}) {
  return (
    <>
...
function StateMappingField({
  ...
  value: Record<string, conductor.LabelSelector>;
  onChange: (v: Record<string, conductor.LabelSelector>) => void;
}) {
  const entries = Object.entries(value);
  const setKey = (oldKey: string, newKey: string) => {
    const next: Record<string, conductor.LabelSelector> = {};
    for (const [k, v] of entries) {
      next[k === oldKey ? newKey : k] = v;
    }
    onChange(next);
  };
  const setSelector = (key: string, sel: conductor.LabelSelector) => {
...
function HookFields({
  ...
  label: string;
  value: conductor.HookSpec | null;
  onChange: (h: conductor.HookSpec | null) => void;
}) {
