  ...
  return (
    <div className="rounded-lg border border-success/40 bg-success-soft p-3 space-y-3">
      <div className="flex items-baseline justify-between gap-2">
        <h3 className="text-label font-semibold text-success-fg">
          Plan materialised
        </h3>
        ...
        <div className="space-y-1.5">
          ...
          <ul className="space-y-1">
            {createdIssues.map((it) => (
              <li
                key={it.id}
                className="text-body flex items-baseline gap-2 rounded bg-surface-1 border border-border-subtle px-2 py-1"
              >
                <code className="text-caption text-fg-subtle shrink-0">
                  {it.id.slice(0, 12)}
                </code>
      ...
      {failedIssues.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-caption uppercase tracking-wide font-medium text-danger-fg">
          ...
          <ul className="space-y-1">
            {failedIssues.map((it, i) => (
              <li
                key={i}
                className="text-micro rounded bg-danger-soft border border-danger/40 px-2 py-1 text-danger-fg"
              >
      ...
      {planPath && (
        <div className="text-micro text-fg-muted">
          Audit markdown:{" "}
          <code className="text-micro font-mono text-fg-default break-all">
            {planPath}
          </code>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-border-subtle">
        {boardEnabled && (
          <Link
            href="/board"
            className="inline-flex items-center gap-1 text-body text-accent-text hover:underline"
          >
...
function DispatcherChip({
  ...
  return (
    <div className="ml-auto flex items-center gap-2">
      {status.error && (
        <span
          className="text-caption text-danger-fg truncate max-w-[180px]"
          title={status.error}
        >
          ⚠ {status.error}
        </span>
      ...
      <span
        className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-micro ${cls}`}
        title={
      ...
      {state === "idle" && hasConfig && (
        <button
          ...
          className="rounded border border-border-default px-2 py-0.5 text-micro hover:bg-surface-2 disabled:opacity-50"
      ...
      {!hasConfig && (
        <>
          <button
            ...
            className="rounded border border-accent/40 bg-accent-soft px-2 py-0.5 text-micro text-accent-text hover:bg-accent-soft/80 disabled:opacity-50"
          ...
          <Link
            href="/dispatcher"
            className="rounded border border-border-default px-2 py-0.5 text-micro text-fg-muted hover:bg-surface-2"
